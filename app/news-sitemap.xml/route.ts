...
export async function GET() {
  ...
  const posts = await client.fetch(
    `*[_type == "post" && !(_id in path("drafts.**")) && defined(slug.current) && defined(publishedAt) && coalesce(firstPublishedAt, publishedAt) >= $cutoff && contentType == "news" && coalesce(workflowStatus, "published") == "published"] | order(coalesce(firstPublishedAt, publishedAt) desc)[0..999]{
      title,
      slug,
